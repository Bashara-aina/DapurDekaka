...
export default async function handler(request: Request): Promise<Response> {
  ...
  if (index === null) {
    return json(error("INVALID_INDEX", "Invalid logo index", 400), 400);
  }
  ...
  try {
    ...
    if (index >= parsed.data.content.customers.logos.length) {
      return json(error("NOT_FOUND", "Customer logo not found", 404), 404);
    }

    parsed.data.content.customers.logos.splice(index, 1);
    await storage.updatePageContent("homepage", { content: parsed.data });
    return json(ok({ message: "Customer logo deleted successfully" }));
  } catch {
    return json(error("DELETE_FAILED", "Failed to delete customer logo", 500), 500);
  }
}
