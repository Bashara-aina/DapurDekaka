  ...
  return (
    <div className="space-y-5 pb-20 md:pb-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-text-primary">{greeting()}, {session?.user?.name ?? 'Admin'} 👋</h1>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
        ...
        <div className="bg-white rounded-card p-5 shadow-card border border-admin-border">
          ...
          {kpis?.systemHealth?.lastCronCheck && (
            <p className="text-xs text-text-disabled mt-1">Cron: {kpis.systemHealth.lastCronCheck}</p>
          )}
        </div>
      </div>
            ...
            <tbody className="divide-y divide-gray-100">
              {auditLogs?.logs?.slice(0, 15).map(log => (
                <tr key={log.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-5 py-3 text-xs font-mono font-medium text-text-primary">{log.action}</td>
