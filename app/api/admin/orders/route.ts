...
import { eq, desc, and, isNull, sql, inArray, gte, lt } from 'drizzle-orm';
...
export async function POST(req: NextRequest) {
  try {
    ...
    const countTodayResult = await db
      .select({ count: sql<number>`count(*)` })
      .from(orders)
      .where(gte(orders.createdAt, new Date(now.getFullYear(), now.getMonth(), now.getDate())))
      .execute();
    const countToday = (countTodayResult[0]?.count ?? 0) + 1;
    const orderNumber = `DDK-${dateStr}-${String(countToday).padStart(4, '0')}`;
