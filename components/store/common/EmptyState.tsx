...
function ActionButton({ action, className }: { action: NonNullable<EmptyStateProps['action']>; className?: string }) {
  if (action.onClick && !action.href) {
    return (
      <button
        onClick={action.onClick}
        className={cn('inline-flex items-center justify-center h-12 px-6 bg-brand-red text-white font-semibold rounded-button shadow-button hover:shadow-button-hover transition-shadow', className)}
      >
        {action.label}
      </button>
    );
