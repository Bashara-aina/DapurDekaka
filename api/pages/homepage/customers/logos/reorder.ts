...
export default async function handler(request: Request): Promise<Response> {
  ...
  try {
    ...
    if (!payload.success) {
      return json(error("VALIDATION_ERROR", "Invalid logos array provided", 400), 400);
    }
    ...
    parsed.data.content.customers.logos = payload.data.logos;
    await storage.updatePageContent("homepage", { content: parsed.data });
    return json(ok({ message: "Customer logos reordered successfully" }));
  } catch {
    return json(error("REORDER_FAILED", "Failed to reorder customer logos", 500), 500);
  }
}
