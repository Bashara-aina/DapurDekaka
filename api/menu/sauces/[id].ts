...
export default async function handler(request: Request): Promise<Response> {
  ...
  if (id === null) {
    return json(error("INVALID_ID", "Invalid sauce ID", 400), 400);
  }
  ...
  if (request.method === "GET") {
    try {
      ...
      if (!sauce) {
        return json(error("NOT_FOUND", "Sauce not found", 404), 404);
      }
  ...
  if (request.method === "PUT") {
    ...
    try {
      ...
      const validation = insertSauceSchema.partial().safeParse(candidate);
