...
import {
  ...
  ClipboardList, AlertTriangle, ShoppingBag, X
} from 'lucide-react';
    ...
    <div className="fixed inset-0 z-50 flex flex-col justify-end lg:items-center lg:justify-center">
      ...
      <div className="relative bg-white rounded-t-2xl lg:rounded-2xl p-5 max-w-md w-full mx-auto shadow-xl max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={onClose} aria-label="Tutup" className="text-gray-400 hover:text-gray-600 transition-colors">
            <X className="w-5 h-5" />
          </button>
        </div>
        {children}
      ...
      >
        <div className="space-y-4">
              ...
              {selectedOrder?.items.map((item, idx) => (
                <label key={idx} className="flex items-center gap-3 cursor-pointer group">
                  <input
                    ...
                    className="w-5 h-5 rounded border-gray-300 text-success focus:ring-success cursor-pointer"
                  />
          ...
          <div>
            ...
            <Select value={coldChain} onValueChange={(v) => setColdChain(v ?? '')}>
              <SelectTrigger className="mt-1 w-full">
                <SelectValue placeholder="Pilih kondisi" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="baik">Baik</SelectItem>
                <SelectItem value="perlu_tambah_es">Perlu Tambah Es</SelectItem>
                <SelectItem value="rusak">Rusak (laporkan ke admin)</SelectItem>
              </SelectContent>
            </Select>
          </div>
        ...
        {[
          ...
          <button
            ...
            className={cn(
              ...
              filterLevel === f.key ? 'bg-admin-sidebar text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
            )}
          >
            {f.label}
          </button>
...
export default function FieldDashboardClient() {
  ...
  return (
    <div className="space-y-4 max-w-xl mx-auto pb-24">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-text-primary">🍜 Dashboard Gudang</h1>
